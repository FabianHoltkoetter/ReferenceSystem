...
@ExposesResourceFor(OrderingItem_.class)
@RequestMapping("/orderingItems/search")
public class OrderingItem_SearchController {
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
	    ...
	    Class tmpClass = OrderingItem_.class;
	    while (tmpClass != null) {
        ...
        Stream<OrderingItem_> orderingItemStream;
        try {
            orderingItemStream = service.query(q, OrderingItem_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            orderingItemStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = orderingItemStream.map(assembler::toResource).collect(Collectors.toList());
