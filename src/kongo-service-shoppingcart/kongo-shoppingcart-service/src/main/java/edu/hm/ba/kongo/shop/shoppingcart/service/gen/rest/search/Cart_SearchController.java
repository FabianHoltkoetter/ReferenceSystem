package edu.hm.ba.kongo.shop.shoppingcart.service.gen.rest.search;
...
@ExposesResourceFor(Cart_.class)
@RequestMapping("/carts/search")
public class Cart_SearchController {
    ...
    public ResponseEntity<?> findFullTextFuzzy(PersistentEntityResourceAssembler assembler, @Param("q") String q) {
        ...
        Stream<Cart_> cartStream;
        try {
            cartStream = service.query(q, Cart_.class, annotatedFields.toArray(new String[annotatedFields.size()])).stream();
        } catch (EmptyQueryException e) {
            cartStream = StreamSupport.stream(repository.findAll().spliterator(), false);
        }

        final List<PersistentEntityResource> collect = cartStream.map(assembler::toResource).collect(Collectors.toList());
