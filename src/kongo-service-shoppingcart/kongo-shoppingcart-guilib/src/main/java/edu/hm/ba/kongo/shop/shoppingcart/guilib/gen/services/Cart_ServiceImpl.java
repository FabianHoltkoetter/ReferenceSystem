...
import edu.hm.ba.kongo.shop.shoppingcart.guilib.services.Cart_FallbackDataGenerator;
...
public class Cart_ServiceImpl implements Cart_Service, Serializable {
    
    private Cart_RestClient client;
    private SecurityService securityService;

    @Autowired
    public Cart_ServiceImpl(SecurityService securityService, @Value("${SHOPPINGCART.microservice.basePath}") String basePath) {
        this.securityService = securityService;
        final URI baseUri = URI.create(basePath);
		this.client = new Cart_RestClientImpl(getTemplate(), baseUri);
    }

	/**
	 * creates one Cart_
	 * @param cart the one who shall be created
	 * @return cart
	 */
    @Override
    ...
    public Cart_ defaultCreate(Cart_ bean) {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.create);
		return Cart_FallbackDataGenerator.createCartFallback();
	}

	/**
	 * updates one Cart_
	 * @param cart the one who shall be updated
	 * @return cart
	 */
    @Override
    ...
    public Cart_ defaultUpdate(Cart_ bean) {
    ...
    public List<Cart_> defaultFindAll() {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
		return Cart_FallbackDataGenerator.createCartsFallback();
	}

	/**
	 * Returns all Cart_ of a specified relation
    ...
    public List<Cart_> defaultFindAll(Link relation) {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
		return Cart_FallbackDataGenerator.createCartsFallback();
	}

	/**
	 * Returns an Optional of Cart_ to one Link
	 * @param link the link
	 * @return the found Cart_ 
	 */
    @Override
    ...
    public Optional<Cart_> defaultFindOne(Link link) {
		showErrorNotification(I18nPaths.NotificationType.error, SimpleAction.read);
		return Cart_FallbackDataGenerator.createOptionalCartFallback();
	}
	 ...
	 */
    @Override
    @HystrixCommand(fallbackMethod = "defaultQueryCart")
    public List<Cart_> queryCart(String query) {
        return client.findFullTextFuzzy(query);
    }
    public List<Cart_> defaultQueryCart(String query) {
		showErrorNotification(I18nPaths.NotificationType.error,SimpleAction.read);
		return Cart_FallbackDataGenerator.createCartsFallback();
	}
    ...
    private void showErrorNotification(I18nPaths.NotificationType type, SimpleAction action) {
        GenericErrorNotification succes = new GenericErrorNotification(
                BaseUI.getCurrentI18nResolver().resolveRelative(Cart_.class, getNotificationPath(type, action, I18nPaths.Type.label)),
                BaseUI.getCurrentI18nResolver().resolveRelative(Cart_.class, getNotificationPath(type, action, I18nPaths.Type.text)));
    ...
    private void showErrorNotification(I18nPaths.NotificationType type, SimpleAction action, String statusCode) {
        GenericErrorNotification succes = new GenericErrorNotification(
                BaseUI.getCurrentI18nResolver().resolveRelative(Cart_.class, getNotificationPath(type, action, I18nPaths.Type.label, statusCode)),
                BaseUI.getCurrentI18nResolver().resolveRelative(Cart_.class, getNotificationPath(type, action, I18nPaths.Type.text, statusCode)));
        succes.show(Page.getCurrent());
    }
}
