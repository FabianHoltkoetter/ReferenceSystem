...
import edu.hm.ba.kongo.shop.shoppingcart.client.businessactions.BusinessActionsRestClientImpl;
 ...
 */
@SpringComponent("shoppingcart_BusinessActionsService")
@UIScope
public class BusinessActionsServiceImpl implements BusinessActionsService {
	 ...
	 */
	@Autowired
	public BusinessActionsServiceImpl(SecurityService securityService, @Value("${SHOPPINGCART.microservice.basePath}") String basePath) {
	...
	@Override
	@HystrixCommand(fallbackMethod = "defaultAddToCart")
	public void addToCart(String productID, Long quantity){
		client.addToCart(productID, quantity);
		LOG.debug("Executed BusinessAction: addToCart " + "With Parameters:" +  " productID: " + productID +  " quantity: " + quantity);
	}
	public void defaultAddToCart(String productID, Long quantity){
		LOG.error("Error during BusinessAction: addToCart " + "With Parameters:" +  " productID: " + productID +  " quantity: " + quantity);
